...
class TCZee(Automaton):
	...
	def established_receive_data(self, pkt):
		...
		if TCP in pkt and (pkt[TCP].flags & 0x10 and pkt[TCP].flags & 0x01):
			...
			self.last_packet = self.l3
			self.send(self.last_packet)
			# TODO 	Trying here to just go again in ESTABLISHED state, as there is an handler 
			# 	for the FIN packets there. Should maybe go to some CLOSING state, for the 
		...
		#	multiple TCP segments, for the moment we assume request fits in one segment
			...
			if pkt[TCP].load: 
				...
				if "GET /" in pkt[TCP].load:
					...
					httpBody = "<html><head><title>Integratio - M. Zunino 2014</title></head><body><h2><span style='font-family:georgia,serif;'>Integratio - new HTTP Test Server</span></h2><p><p><span style='color:#800080;'><span style='font-size:14px;'><em><span style='font-family:georgia,serif;'>The Integration project is an Open Source project started in 2014 by Marco Zunino.&nbsp;<a href='https://github.com/zupino/tcz'>https://github.com/zupino/tcz</a></span></em></span></span></p><span style='font-family:georgia,serif;'>This is a&nbsp;<em>really simple, </em><a href='https://bitbucket.org/secdev/scapy/src'>Scapy</a>-based implementation of a HTTP Web Server. The scope of this tool is to provide a framework to test the client robustness to <strong>network delay</strong>, <strong>malformed response </strong>and <strong>error condition</strong>.</span></p></body></html>"
			...
			# DEBUG
                	...
                	self.last_packet = self.l3/httpResponse
               	 	self.send(self.last_packet)
        	       	
 			# sending the actual HTTP response
