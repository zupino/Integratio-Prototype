...
class TCZee(Automaton):
	...
	def receive_syn(self, pkt):
		#DEBUG	
		print "\t[DEBUG][LISTEN] Entering in the receive_syn method"
	...
	def receive_finAck(self, pkt):
		...
		if TCP in pkt and (pkt[TCP].sport == self.dport) and (pkt[TCP].flags & 0x10 and pkt[TCP].flags & 0x01):
			...
			# DEBUG
			print "\t\t[DEBUG][ESTABLISHED] in the condition, pkt is a FIN so I raise CLOSING state (condition): " + pkt.summary()
		...
		if TCP in pkt and (pkt[TCP].sport == self.dport) and (pkt[TCP].flags & 0x10) and (pkt[TCP].flags & 0x08):
			...
			if pkt[TCP].load and (pkt[TCP].load != self.lastHttpRequest) : 
				...
				if "GET /delay.html HTTP" in pkt[TCP].load:
					...
					httpBody = "<h1>TCZee - Example of basic HTTP Server</h1><p><strong>TCZee</strong> is a <em>basic TCP stack implementation using Scapy</em> aimed to provide a framework for the creation of networking-related test cases, helping to re-create fault condition <em>impossible to trigger</em> in a production environment.</p><p>It allows to keep <strong>full control</strong> on the network layer and <em>simulate the remote services</em> of test client IoT products to investigate its robustness, standard compliance and security.<br /><em>Here 2 simple test case examples:</em></p><p><a href='delay.html'>- Get a valid page with 10 seconds delay</a><br /><a href='567error.html'>- Get a 567 HTTP Errorresponse</a></p><p><strong>TCZee</strong> is part of the Integratio Project, created in 2014 by Marco Zunino.<br /><a href='https://github.com/zupino/tcz'> https://github.com/zupino/tcz</a> - eng.marco.zunino@gmail.com</p>"
